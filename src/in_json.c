...
#define FIELD_HANDLE(nam, handle_code, dxf)                                   \
  ...
  {                                                                           \
    fprintf (g_dat->fh, "\"%s\": \"HANDLE(%d.%d.%lu) absolute:%lu\",\n",      \
             #nam, _obj->nam->handleref.code, _obj->nam->handleref.size,      \
             _obj->nam->handleref.value, _obj->nam->absolute_ref);            \
  }
...
#define XDICOBJHANDLE(code)                                                   \
  ...
  PRIOR_VERSIONS { VALUE_HANDLE (obj->tio.object->xdicobjhandle, code, 330); }
